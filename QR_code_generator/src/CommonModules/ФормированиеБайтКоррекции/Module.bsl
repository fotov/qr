Функция БайтыКоррекции(Байты, КоличествоБайтКоррекции) Экспорт
	...
	МассивКоррекции = Новый БуферДвоичныхДанных(Макс(КоличествоБайтКоррекции, Байты.Размер));
	МассивКоррекции.Записать(0, Байты);
	
	Для НомерБайта = 1 По Байты.Размер Цикл
	
		ЗначениеТекущегоЭлемента = МассивКоррекции[0];
		МассивКоррекции.Записать(0, МассивКоррекции.ПолучитьСрез(1, МассивКоррекции.Размер - 1));
		МассивКоррекции[МассивКоррекции.Размер - 1] = 0;
		...
		Для ИндексКоррекции = 0 По ГенерирующийМногочлен.Количество() - 1 Цикл
			...
			МассивКоррекции[ИндексКоррекции] = ПобитовоеИсключительноеИли(ЭлементГалуа, МассивКоррекции[ИндексКоррекции]);
		
		КонецЦикла; 
	
	КонецЦикла;
	
	Возврат МассивКоррекции.ПолучитьСрез(0, КоличествоБайтКоррекции);
